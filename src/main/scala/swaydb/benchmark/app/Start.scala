...
import swaydb.IO
import swaydb.benchmark.app.{RunnerAPI, RunnerZero}
...
object Start {
  ...
  def main(args: Array[String]): Unit = {
    ...
    println
    print("Enter map size (in bytes) (hit Enter for 20.mb): ")
    val mapSize = Try(readInt()) getOrElse 20.mb

    println
    print("Enter segment size (in bytes) (hit Enter for 10.mb): ")
    val segmentSize = Try(readInt()) getOrElse 10.mb
